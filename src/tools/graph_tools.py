...
def doc_tool(state: State) -> Literal["rewrite", "generate", "web_search"]:
    ...
    Falls back to web_search after 2 failed rewrite attempts to prevent infinite loops.

    Args:
        state (State): The current state of the graph.

    Returns:
        The next node: "generate" if score is "yes", "web_search" if retries exhausted,
        otherwise "rewrite".
    """
    ...
    if score == "yes":
        ...
        print("[doc_tool] Max retries reached, falling back to web_search")
        return "web_search"
    else:
        return "rewrite"
